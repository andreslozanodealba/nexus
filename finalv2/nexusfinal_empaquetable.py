...
from PyQt6.QtGui import QFont, QTextDocument, QIcon
...
# Directorio local único donde se guarda la base de datos de la aplicación.
# Todos los usuarios de este mismo PC (empleados y admin) comparten esta carpeta.
USER_DATA_DIR = "/opt/nexus-facturacion/data"

# Identidad visual de la aplicación. En una instalación mediante .deb
# se usa el icono instalado en /usr/share; al ejecutar el .py directamente
# se intenta usar el icono junto al archivo fuente.
APP_NAME = "Nexus Facturación"
APP_DESKTOP_ID = "nexus-facturacion"
APP_ICON_SYSTEM = "/usr/share/icons/hicolor/256x256/apps/nexus-facturacion.png"
APP_ICON_LOCAL = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nexus-icon.png")
APP_ICON_PATH = APP_ICON_SYSTEM if os.path.exists(APP_ICON_SYSTEM) else APP_ICON_LOCAL
...
class VerFacturaTxtDialog(QDialog):
  """Vista de factura. Solo cambia la presentación; usa la BDD existente."""
  ...
  def __init__(self, fac_id, username="", parent=None):
    ...
    self.setStyleSheet(STYLESHEET_CYBER_MEDICO + """
      ...
      QFrame#invoice_header {
        background-color: #07080f;
        border-bottom: 1px solid #3b82f6;
      }
      QFrame#invoice_info, QFrame#invoice_totals {
        background-color: #07080f;
        border: 1px solid #1f2041;
      }
      QLabel#invoice_brand {
        color: #60a5fa;
        font-size: 22px;
        font-weight: 700;
        letter-spacing: 1px;
      }
      QLabel#invoice_subtitle {
        color: #94a3b8;
        font-size: 10px;
      }
      QLabel#invoice_title {
        color: #e0f2fe;
        font-size: 20px;
        font-weight: 700;
      }
      ...
      QLabel#invoice_value {
        color: #e0f2fe;
        font-size: 12px;
      }
      QTableWidget#invoice_table {
        background-color: #07080f;
        color: #e0f2fe;
        border: 1px solid #3b82f6;
        gridline-color: #1f2041;
        selection-background-color: #07080f;
        selection-color: #e0f2fe;
      }
      QTableWidget#invoice_table::item {
        padding: 7px;
      }
      QHeaderView::section {
        background-color: #1e3a8a;
      ...
      QLabel#invoice_total_label {
        color: #e0f2fe;
        font-size: 14px;
        font-weight: 700;
      }
      QLabel#invoice_total {
        color: #60a5fa;
        font-size: 18px;
        font-weight: 700;
      }
      QLabel#invoice_thanks {
        color: #94a3b8;
        font-size: 11px;
      }
    """)
    ...
    page = QFrame()
    page.setObjectName("invoice_page")
    page_lay = QVBoxLayout(page)
    page_lay.setContentsMargins(18, 18, 18, 18)
    page_lay.setSpacing(12)

    header = QFrame()
    header.setObjectName("invoice_header")
    header_lay = QHBoxLayout(header)
    header_lay.setContentsMargins(12, 12, 12, 12)

    brand_box = QVBoxLayout()
    brand = QLabel("NEXUS")
    brand.setObjectName("invoice_brand")
    brand_box.addWidget(brand)
    subtitle = QLabel("SISTEMA DE GESTIÓN ADMINISTRATIVA")
    subtitle.setObjectName("invoice_subtitle")
    brand_box.addWidget(subtitle)
    header_lay.addLayout(brand_box)
    header_lay.addStretch()

    right = QVBoxLayout()
    title = QLabel("FACTURA")
    title.setObjectName("invoice_title")
    title.setAlignment(Qt.AlignmentFlag.AlignRight)
    right.addWidget(title)
    self.lbl_numero = QLabel(f"N° {fac_id}")
    self.lbl_numero.setObjectName("invoice_number")
    self.lbl_numero.setAlignment(Qt.AlignmentFlag.AlignCenter)
    right.addWidget(self.lbl_numero)
    header_lay.addLayout(right)
    page_lay.addWidget(header)

    self.info = QFrame()
    self.info.setObjectName("invoice_info")
    info_lay = QHBoxLayout(self.info)
    info_lay.setContentsMargins(12, 10, 12, 10)

    self.lbl_emisor = QLabel()
    self.lbl_emisor.setObjectName("invoice_value")
    self.lbl_fecha = QLabel()
    self.lbl_fecha.setObjectName("invoice_value")
    self.lbl_cliente = QLabel()
    self.lbl_cliente.setObjectName("invoice_value")

    left_info = QVBoxLayout()
    left_info.addWidget(QLabel("DATOS DE EMISIÓN", objectName="section_title"))
    left_info.addWidget(self.lbl_emisor)
    left_info.addWidget(self.lbl_fecha)
    info_lay.addLayout(left_info)
    info_lay.addStretch()

    client_info = QVBoxLayout()
    client_info.addWidget(QLabel("CLIENTE", objectName="section_title"))
    client_info.addWidget(self.lbl_cliente)
    info_lay.addLayout(client_info)
    page_lay.addWidget(self.info)

    lbl_productos = QLabel("DESCRIPCIÓN DE PRODUCTOS")
    lbl_productos.setObjectName("section_title")
    page_lay.addWidget(lbl_productos)

    self.tabla_factura = QTableWidget()
    self.tabla_factura.setObjectName("invoice_table")
    self.tabla_factura.setColumnCount(4)
    self.tabla_factura.setHorizontalHeaderLabels(
    ...
    self.tabla_factura.setWordWrap(True)
    self.tabla_factura.verticalHeader().setVisible(False)
    self.tabla_factura.verticalHeader().setSectionResizeMode(
        QHeaderView.ResizeMode.ResizeToContents
    )
    self.tabla_factura.horizontalHeader().setSectionResizeMode(
    ...
    page_lay.addWidget(self.tabla_factura, 1)

    totals = QFrame()
    totals.setObjectName("invoice_totals")
    totals_lay = QVBoxLayout(totals)
    totals_lay.setContentsMargins(12, 10, 12, 10)
    total_row = QHBoxLayout()
    total_label = QLabel("TOTAL A PAGAR")
    total_label.setObjectName("invoice_total_label")
    self.lbl_total = QLabel("$0.00")
    self.lbl_total.setObjectName("invoice_total")
    self.lbl_total.setAlignment(Qt.AlignmentFlag.AlignRight)
    total_row.addWidget(total_label)
    total_row.addStretch()
    total_row.addWidget(self.lbl_total)
    totals_lay.addLayout(total_row)
    page_lay.addWidget(totals)

    thanks = QLabel("Comprobante generado por Nexus.")
    thanks.setObjectName("invoice_thanks")
    thanks.setAlignment(Qt.AlignmentFlag.AlignCenter)
    page_lay.addWidget(thanks)
    ...
    if self.username:
      ...
      for f in db.get("facturas", []):
        if str(f.get("id")) == str(self.fac_id):
          ...
          total = str(f.get("total", total)).replace("$", "")
          items = f.get("items", [])
          break

    self.lbl_emisor.setText(
        f"Emitida por: {self.username.upper() if self.username else 'SISTEMA'}"
    )
    self.lbl_fecha.setText(f"Fecha/Hora: {fecha}")
    self.lbl_cliente.setText(f"{cliente}")

    self.tabla_factura.setRowCount(0)
    for item in items:
      row = self.tabla_factura.rowCount()
      self.tabla_factura.insertRow(row)
      ...
      self.tabla_factura.setItem(row, 0, QTableWidgetItem(nombre))
      self.tabla_factura.setItem(row, 1, QTableWidgetItem(str(cantidad)))
      self.tabla_factura.setItem(row, 2, QTableWidgetItem(f"${precio:,.2f}"))
      self.tabla_factura.setItem(row, 3, QTableWidgetItem(f"${subtotal:,.2f}"))

    if not items:
      row = self.tabla_factura.rowCount()
      self.tabla_factura.insertRow(row)
      self.tabla_factura.setItem(row, 0, QTableWidgetItem("Consulta / Servicio General"))
      self.tabla_factura.setItem(row, 1, QTableWidgetItem("1"))
      self.tabla_factura.setItem(row, 2, QTableWidgetItem(f"${float(total or 0):,.2f}"))
      self.tabla_factura.setItem(row, 3, QTableWidgetItem(f"${float(total or 0):,.2f}"))

    try:
      total_num = float(str(total).replace(",", ""))
      self.lbl_total.setText(f"${total_num:,.2f}")
    except (TypeError, ValueError):
    ...
    if archivo_guardar:
      try:
        lines = [
            ...
            "=" * 58,
            f"N° de Factura : {self.fac_id}",
            f"Emitida por   : {self.username.upper() if self.username else 'SISTEMA'}",
            f"Fecha/Hora    : {self.lbl_fecha.text().split(': ', 1)[-1]}",
            f"Cliente       : {self.lbl_cliente.text()}",
            "-" * 58,
            "DESCRIPCIÓN                         CANT  VALOR",
            "-" * 58,
        ]
        for r in range(self.tabla_factura.rowCount()):
          vals = [self.tabla_factura.item(r, c).text() if self.tabla_factura.item(r, c) else "" for c in range(4)]
          lines.append(f"{vals[0]:<35} {vals[1]:<5} {vals[3]}")
        lines.extend([
            "-" * 58,
            f"TOTAL A PAGAR                    {self.lbl_total.text()}",
            "=" * 58,
        ...
        with open(archivo_guardar, "w", encoding="utf-8") as f_out:
          f_out.write("\n".join(lines))
        QMessageBox.information(
            self, "Éxito", f"Factura exportada correctamente en:\n{archivo_guardar}"
        )
  ...
  def exportar_pdf(self):
    ...
    if archivo_guardar:
      try:
        ...
        for r in range(self.tabla_factura.rowCount()):
          vals = [self.tabla_factura.item(r, c).text() if self.tabla_factura.item(r, c) else "" for c in range(4)]
          safe = [v.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;") for v in vals]
          rows_html += (
              f"<tr>"
              f"<td style='text-align:left;'>{safe[0]}</td>"
              f"<td style='text-align:center;'>{safe[1]}</td>"
              f"<td style='text-align:right;'>{safe[2]}</td>"
              f"<td style='text-align:right;'>{safe[3]}</td>"
              f"</tr>"
          )

        html_content = f"""
        <html><head><style>
          body {{ background:#ffffff; color:#111111; font-family:Helvetica, Arial, sans-serif; margin:0; padding:15px; }}
          .invoice-box {{ max-width: 700px; margin: auto; padding: 20px; border: 1px solid #d1d5db; background: #ffffff; }}
          .header-table, .details-table, .items-table {{ width: 100%; border-collapse: collapse; }}
          .header-table td {{ vertical-align: top; padding: 0; }}
          .brand {{ color: #1e3a8a; font-size: 24px; font-weight: bold; }}
          .sub {{ color: #6b7280; font-size: 11px; margin-top: 2px; }}
          .title-section {{ text-align: right; }}
          .title {{ color: #111827; font-size: 22px; font-weight: bold; }}
          .num {{ color: #1e3a8a; font-size: 13px; font-weight: bold; margin-top: 4px; }}
          .info-box {{ margin-top: 20px; margin-bottom: 20px; padding: 12px; background: #f9fafb; border: 1px solid #e5e7eb; font-size: 12px; line-height: 1.5; }}
          .items-table th {{ background: #1e3a8a; color: #ffffff; padding: 8px 10px; font-size: 12px; border: 1px solid #1e3a8a; }}
          .items-table td {{ padding: 8px 10px; font-size: 12px; border: 1px solid #e5e7eb; color: #374151; }}
          .total-box {{ margin-top: 15px; width: 100%; border-collapse: collapse; }}
          .total-box td {{ padding: 10px; font-size: 14px; font-weight: bold; background: #f3f4f6; border: 1px solid #d1d5db; text-align: right; }}
          .thanks {{ text-align: center; color: #9ca3af; font-size: 11px; margin-top: 25px; }}
        </style></head><body>
          <div class="invoice-box">
            <table class="header-table">
              <tr>
                <td>
                  ...
                  <div class="sub">SISTEMA DE GESTIÓN ADMINISTRATIVA</div>
                </td>
                <td class="title-section">
                  <div class="title">FACTURA</div>
                  <div class="num">N° {self.fac_id}</div>
                </td>
              </tr>
            </table>
            ...
            <div class="info-box">
              ...
              Emitida por: {self.username.upper() if self.username else 'SISTEMA'}<br>
              {self.lbl_fecha.text()}<br><br>
              <strong>CLIENTE:</strong> {self.lbl_cliente.text()}
            </div>
            ...
            <table class="total-box">
              <tr>
                <td>TOTAL A PAGAR: {self.lbl_total.text()}</td>
              </tr>
            </table>

            <div class="thanks">Comprobante generado por Nexus[cite: 3, 4].</div>
          </div>
        </body></html>
        """
        ...
        QMessageBox.information(
            self, "Éxito", f"Factura exportada a PDF correctamente en:\n{archivo_guardar}"
        )
      except Exception as e:
        QMessageBox.warning(self, "Error", f"No se pudo exportar el PDF: {e}")
...
class TablaArrastre(QTableWidget):
    """QTableWidget con desplazamiento por arrastre del contenido."""

    def __init__(self, parent=None):
        super().__init__(parent)
        self._arrastrando = False
        self._pos_arrastre = QPoint()

    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            self._arrastrando = True
            self._pos_arrastre = event.position().toPoint()
            self.setCursor(Qt.CursorShape.ClosedHandCursor)
        super().mousePressEvent(event)

    def mouseMoveEvent(self, event):
        if self._arrastrando and (event.buttons() & Qt.MouseButton.LeftButton):
            pos_actual = event.position().toPoint()
            delta = pos_actual - self._pos_arrastre
...
class EditarRegistroDialog(QDialog):
  def __init__(self, titulo, campos, editables=None, parent=None):
    super().__init__(parent)
    self.setWindowTitle(titulo)
    self.setMinimumSize(420, 260)
    self.setStyleSheet(STYLESHEET_CYBER_MEDICO)
    self.editables = set(editables or [])
    self.inputs = {}
    ...
    for clave, valor in campos:
      inp = QLineEdit(str(valor))
      inp.setReadOnly(clave not in self.editables)
      self.inputs[clave] = inp
      form.addRow(QLabel(f"{clave}:"), inp)
  ...
  def valores(self):
    return {clave: inp.text() for clave, inp in self.inputs.items()}
...
class NexusApp(QMainWindow):
  ...
  def init_ui(self):
    ...
    lbl_foot = QLabel("NEXUS // SISTEMA MULTI-USUARIO CONECTADO")
  ...
  def configurar_tabla(self, tabla, indice):
    ...
    tabla.setTextElideMode(Qt.TextElideMode.ElideNone)
    ...
    tabla.verticalHeader().setSectionResizeMode(
        QHeaderView.ResizeMode.ResizeToContents
    )
    if indice in [1, 3]:
      tabla.setStyleSheet(
  ...
  def crear_vista_clientes(self):
    ...
    self.t_cli = TablaArrastre()
    self.t_cli.cellDoubleClicked.connect(self.ver_cliente_doble_click)
  ...
  def ver_cliente_doble_click(self, row, column):
    item = self.t_cli.item(row, 0)
    if not item:
      return
    cliente_id = item.text()
    db = cargar_db_usuario(self.username)
    cliente = next(
      (c for c in db.get("clientes", []) if str(c.get("id")) == cliente_id),
      None
    )
    if not cliente:
      return
    ...
    EditarRegistroDialog("Ver Cliente", campos, editables=[], parent=self).exec()

  def ver_editar_producto_doble_click(self, row, column):
    item = self.t_inv.item(row, 0)
    if not item:
      return
    prod_id = item.text()
    db = cargar_productos_global()
    producto = next(
      (p for p in db.get("inventario", []) if str(p.get("id")) == prod_id),
      None
    )
    if not producto:
      return
    ...
    if dlg.exec() == QDialog.DialogCode.Accepted:
      nuevo_nombre = dlg.valores()["Nombre"].strip()
      if not nuevo_nombre:
  ...
  def ver_factura_doble_click(self, row, column):
    item = self.t_fac.item(row, 0)
    if item:
      self.ver_factura_archivo(item.text())
  ...
  def crear_vista_inventario(self):
    ...
    self.t_inv = TablaArrastre()
    self.t_inv.cellDoubleClicked.connect(self.ver_editar_producto_doble_click)
  ...
  def crear_vista_facturacion(self):
    ...
    self.t_fac = TablaArrastre()
    self.t_fac.cellDoubleClicked.connect(self.ver_factura_doble_click)
  ...
  def abrir_modal_factura(self):
    ...
    if dlg.exec() == QDialog.DialogCode.Accepted:
      ...
      if facturas:
        self.ver_factura_archivo(str(facturas[-1].get("id", "")))
